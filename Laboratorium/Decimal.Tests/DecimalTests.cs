...
public class DecimalHelperTests
{
    ...
    public void NegativeZero_IsNegative_ReturnsFalse()
    {
        ...
        // Assert
        Assert.True(negativeZeroIsNegative, "decimal.IsNegative(-0.000m) should return true");
    ...
    [Fact]
    public void DecimalNegativeZero_ScaleChangesIsNegativeResult()
    {
        // -0m and -0.000m are both decimal literals, but the compiler gives them different scales.
        // Decimal stores the sign and the scale inside the flags field.
        // Because the scale is different (0 vs 3), the internal flags value is different.
        // IsNegative reads only the sign bit from flags, so the two zeros are not treated the same.
        Assert.True(decimal.IsNegative(-0m) != decimal.IsNegative(-0.000m));
    }
